...
int main(void)
{
    ...
    std::vector<double> exp_vals = { 0.07961459,  0.1140927,  0.1226265 };
    ...
    STATS_TEST_EXPECTED_VAL(dgamma,inp_vals[2],exp_vals[2],false,shape_par,scale_par);
    STATS_TEST_EXPECTED_VAL(dgamma,inp_vals[1],exp_vals[1],true,shape_par,scale_par);
    ...
    STATS_TEST_EXPECTED_VAL(dgamma,0,TEST_POSINF,false,0.5,3);                                      // x == 0
    STATS_TEST_EXPECTED_VAL(dgamma,0,1/scale_par,false,1,scale_par);
    STATS_TEST_EXPECTED_VAL(dgamma,0,0,false,2,3);

    STATS_TEST_EXPECTED_VAL(dgamma,TEST_POSINF,0,false,TEST_POSINF,TEST_POSINF);                    // x == +Inf or shape == +Inf or scale == +Inf
    STATS_TEST_EXPECTED_VAL(dgamma,TEST_POSINF,0,false,2,2);
    STATS_TEST_EXPECTED_VAL(dgamma,2,0,false,TEST_POSINF,2);
    STATS_TEST_EXPECTED_VAL(dgamma,2,0,false,2,TEST_POSINF);
 
    //
    // vector/matrix tests

#ifdef STATS_TEST_STDVEC_FEATURES
    STATS_TEST_EXPECTED_MAT(dgamma,inp_vals,exp_vals,std::vector<double>,false,shape_par,scale_par);
    STATS_TEST_EXPECTED_MAT(dgamma,inp_vals,exp_vals,std::vector<double>,true,shape_par,scale_par);
#endif
    ...
    STATS_TEST_EXPECTED_MAT(dgamma,inp_mat,exp_mat,mat_obj,false,shape_par,scale_par);
    STATS_TEST_EXPECTED_MAT(dgamma,inp_mat,exp_mat,mat_obj,true,shape_par,scale_par);
#endif
