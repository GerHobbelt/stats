...
int main(void)
{
    ...
    std::vector<double> inp_vals = { 2.0,         1.0,        0.2 };
    std::vector<double> exp_vals = { 0.09856858,  0.1760327,  0.4257966 };
    ...
    STATS_TEST_EXPECTED_VAL(dlnorm,1.0,TEST_NAN,false,TEST_NAN,1);
    STATS_TEST_EXPECTED_VAL(dlnorm,1.0,TEST_NAN,false,0,TEST_NAN);

    STATS_TEST_EXPECTED_VAL(dlnorm,-1,0,false,1,2);                                                 // x < 0
    STATS_TEST_EXPECTED_VAL(dlnorm,TEST_NEGINF,0,false,1,2);
    ...
    STATS_TEST_EXPECTED_VAL(dlnorm,TEST_POSINF,TEST_POSINF,false,TEST_POSINF,0);                    // sigma > 0 and log(x) == mu == Inf
    STATS_TEST_EXPECTED_VAL(dlnorm,TEST_POSINF,TEST_NAN,false,TEST_POSINF,1);
    STATS_TEST_EXPECTED_VAL(dlnorm,0,TEST_POSINF,false,TEST_NEGINF,0);                              // sigma > 0 and log(x) == mu == -Inf
    ...
    STATS_TEST_EXPECTED_VAL(dlnorm,0,0,false,TEST_POSINF,0);                                        // x == 0, mu == Inf
    STATS_TEST_EXPECTED_VAL(dlnorm,0,0,false,TEST_POSINF,1);

    STATS_TEST_EXPECTED_VAL(dlnorm,1,0,false,0,0);                                                  // sigma == 0; x > 0 and x,mu finite
