...
int main(void)
{
    ...
    std::vector<double> exp_vals = { 0.8646647,  0.9816844,  0.9975212 };

    //
    // scalar tests

    STATS_TEST_EXPECTED_VAL(pexp,inp_vals[0],exp_vals[0],false,rate);
    STATS_TEST_EXPECTED_VAL(pexp,inp_vals[1],exp_vals[1],true,rate);
    STATS_TEST_EXPECTED_VAL(pexp,inp_vals[2],exp_vals[2],false,rate);

    STATS_TEST_EXPECTED_VAL(pexp,TEST_NAN,TEST_NAN,false,3);                                        // Input NaNs
    STATS_TEST_EXPECTED_VAL(pexp,1,TEST_NAN,false,TEST_NAN);
    STATS_TEST_EXPECTED_VAL(pexp,TEST_NAN,TEST_NAN,false,TEST_NAN);

    STATS_TEST_EXPECTED_VAL(pexp,0,0,false,0);                                                      // rate <= 0
    STATS_TEST_EXPECTED_VAL(pexp,1,0,false,0);
    ...
    STATS_TEST_EXPECTED_MAT(pexp,inp_vals,exp_vals,std::vector<double>,false,rate);
    STATS_TEST_EXPECTED_MAT(pexp,inp_vals,exp_vals,std::vector<double>,true,rate);
#endif
    ...
    STATS_TEST_EXPECTED_MAT(pexp,inp_mat,exp_mat,mat_obj,false,rate);
